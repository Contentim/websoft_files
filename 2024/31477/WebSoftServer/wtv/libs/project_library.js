 ...
 * @property {string} errorText – текст ошибки
 * @property {oTaskType[]} array – Массив заявок.
*/
/**
...
function get_task_types()
{
	...
	*/
	...
	for( _task_type in XQuery( "for $elem in task_types return $elem" ) )
	{
		obj = new Object();
		obj.id = _task_type.id.Value;
		obj.name = _task_type.name.Value;
...
function get_tasks( iPersonID, tePerson, sType, dCreateDateStart, dCreateDateFinish, iAssignerID, iTaskTypeID, iProjectID, sStatus, sSearch, sAccessRole, aPersons, bReturnCount )
{
	...
	*/
	...
	{
		set_error( 1, "Передан некорректный ID сотрудника", false )
		return oRes;
	}
	...
	{
		set_error( 1, "Передан некорректный тип выборки задач", false )
		return oRes;
	}
	try
	{
	...
	switch( sType )
	{
		...
		case "persons":
			...
			{
				set_error( 1, "Не передан массив сотрудников.", false )
				return oRes;
			}
	...
	switch( sCommand )
	{
		case "eval":
			...
			oRes.action_result = {
				...
				title: ( teTask != null ? teTask.name.Value : "Новая задача" ),
				header: "Заполните поля задачи",
				form_fields: [] 
			};
			...
			add_field( "name", "Название", "string", "name", false, true );
			
			if( iTaskTypeID == null )
			{
				if( teTask == null || ( bBoss || bAssigner ) )
				{
					oRes.action_result.form_fields.push( { name: "task_type_id", label: "Тип задачи", type: "foreign_elem", catalog: "task_type", value: ( teTask != null ? teTask.task_type_id.Value : "" ), display_value: ( teTask != null ? get_field_name_value( teTask.task_type_id ) : "" ), mandatory: false } );
				}
				else
				{
					oRes.action_result.form_fields.push( { name: "task_type_id" + "_paragraph", type: "paragraph", value: "Тип задачи" + " - " + ( get_field_name_value( ( teTask != null ? teTask.task_type_id : "" ) ) ) } );
				...
				switch( _field )
				{
					case "executor_id":
						if( iExecutorID == null )
						{
							...
							{
								field = { name: "executor_id", label: "Ответственный", type: "select", value: ( teTask != null ? teTask.executor_id.Value : "" ), mandatory: false, entries: [] };
								bOk = false;
							...
							if( teTask == null || ( bBoss || bAssigner ) )
							{
								...
								{
									oRes.action_result.form_fields.push( { name: "executor_id", label: "Ответственный", type: "foreign_elem", catalog: "collaborator", value: ( teTask != null ? teTask.executor_id.Value : "" ), display_value: ( teTask != null ? get_field_name_value( teTask.executor_id ) : "" ), mandatory: false } );
								}
							}
							else
							{
								oRes.action_result.form_fields.push( { name: "executor_id" + "_paragraph", type: "paragraph", value: "Ответственный" + " - " + ( teTask != null ? get_field_name_value( teTask.executor_id ) : "" ) } );
						...
						{
							oRes.action_result.form_fields.push( { name: "executor_id", label: "Ответственный", type: "hidden", value: iExecutorID } );
						}
						break;
					
					case "start_date_plan":
						add_field( "start_date_plan", "Планируемая дата начала", "date", "start_date_plan", true );
						break;
					case "status":
						field = { name: "status", label: "Статус", type: "select", value: ( teTask != null ? teTask.status.Value : "" ), mandatory: true, entries: [], column: 2 };
						for( _status in common.assessment_appraise_statuses )
						{
					...
					case "end_date_plan":
						add_field( "end_date_plan", "Планируемая дата завершения", "date", "end_date_plan", true);
						break;
					
					case "desc":
						add_field( "desc", "Описание", "text", "desc", false );
						break;
					
					case "file_id":
						if( teTask == null || ( bBoss || bAssigner || bExecutor ) )
						{
							oRes.action_result.form_fields.push( { name: "file_id", label: "Файл", type: "file", value: ( teTask != null ? ( teTask.files.ChildNum > 1 ? ArrayOptFirstElem( teTask.files ).file_id.Value : "" ) : "" ), display_value: get_field_name_value( ( teTask != null ? ( teTask.files.ChildNum > 1 ? ArrayOptFirstElem( teTask.files ).file_id.Value : "" ) : "" ) ), mandatory: false } );
						}
						else
						{
							oRes.action_result.form_fields.push( { name: "file_id" + "_paragraph", type: "paragraph", value: "Файл" + " - " + ( get_field_name_value( ( teTask != null ? ( teTask.files.ChildNum > 1 ? ArrayOptFirstElem( teTask.files ).file_id.Value : "" ) : "" ) ) ) } );
					...
					case "date_plan":
						if( teTask != null && ( bBoss || bAssigner ) )
						{
							oRes.action_result.form_fields.push( { name: "date_plan", label: "Фактическая дата начала", type: "date", value: ( teTask != null ? teTask.date_plan.Value : "" ), mandatory: false, column: 2 } );
						}
						break;
					case "date_fact":
						if( teTask != null && ( bBoss || bAssigner ) )
						{
							oRes.action_result.form_fields.push( { name: "date_fact", label: "Фактическая дата завершения", type: "date", value: ( teTask != null ? teTask.date_fact.Value : "" ), mandatory: false, column: 2 } );
						}
						break;
					case "plan_labor_costs":
						add_field( "plan_labor_costs", "Планируемые трудозатраты", "integer", "plan_labor_costs", true );
						break;
					case "fact_labor_costs":
						if( teTask != null && ( bBoss || bAssigner ) )
						{
							oRes.action_result.form_fields.push( { name: "fact_labor_costs", label: "Фактические трудозатраты", type: "integer", value: ( teTask != null ? teTask.fact_labor_costs.Value : "" ), mandatory: false, column: 2 } );
						}
						break;
					case "value":
						if( teTask != null && ( bBoss || bAssigner ) )
						{
							oRes.action_result.form_fields.push( { name: "value", label: "Оценка", type: "integer", value: ( teTask != null ? teTask.value.Value : "" ), mandatory: false} );
						}
						break;
					case "parent_task_id":
						if( ( bBoss || bAssigner ) )
						{
							oRes.action_result.form_fields.push( { name: "parent_task_id", label: "Родительская задача", type: "foreign_elem", catalog: "task", value: ( teTask != null ? teTask.parent_task_id.Value : "" ), display_value: get_field_name_value( ( teTask != null ? teTask.parent_task_id : "" ) ), mandatory: false, query_qual: ( iSourceObjectID != null ? "$elem/source_object_id = " + iSourceObjectID : "" ) } );
						}
						break;
					case "comment":
						if( teTask != null && ( bBoss || bAssigner || bExecutor ) )
						{
							oRes.action_result.form_fields.push( { name: "comment", label: "Комментарий", type: "text", value: ( teTask != null ? teTask.comment.Value : "" ), mandatory: false } );
						}
						break;
					case "fact":
						if( teTask != null && ( bBoss || bAssigner || bExecutor ) )
						{
							oRes.action_result.form_fields.push( { name: "fact", label: "Решение", type: "text", value: ( teTask != null ? teTask.fact.Value : "" ), mandatory: false } );
						}
						break;
		...
		case "submit_form":
			...
			if( oValue != undefined && OptInt( oValue.value ) != undefined && OptInt( SCOPE_WVARS.GetOptProperty( "iMaxMark" ) ) != undefined )
			{
				...
				{
					oRes.action_result = { command: "alert", msg: "Максимальная оценка по задаче " + OptInt( SCOPE_WVARS.GetOptProperty( "iMaxMark" ) ) };
					break;
				}
			}
			if( iExecutorID == undefined && false )
			{
				oRes.action_result = { command: "alert", msg: "Не передан ответственный по задаче" };
				break;
			}
			if( iTaskTypeID == null )
			{
				...
				if( iTaskTypeID != null )
				{
					...
					if( ArrayOptFirstElem( fldWebCustomFields ) != undefined )
					{
						oRes.action_result = {
							command: "display_form",
							title: "Новая задача",
							header: "Заполните дополнительные поля задачи",
							form_fields: [] 
						};
				...
				if( bCanCheckCustomFields )
				{
					...
					if ( oResCE.error != 0 )
					{
						if ( ArrayCount( oResCE.mandatory_fields ) != 0 )
							sErrorText += StrReplace( tools_web.get_web_const( 'neobhodimozapo_1', curLngWeb ), '{PARAM1}', ArrayMerge( oResCE.mandatory_fields, 'tools_web.get_cur_lng_name(This.title,curLng.short_id)', '", "' ) );
						if ( ArrayCount( oResCE.condition_fields ) != 0 )
							sErrorText += ' ' + StrReplace( tools_web.get_web_const( 'nevernyeznachen', curLngWeb ), '{PARAM1}', ArrayMerge( oResCE.condition_fields, 'tools_web.get_cur_lng_name(This.title,curLng.short_id)', '", "' ) );
			...
			oRes.action_result = { command: "close_form", msg: "Задача успешно создана", confirm_result: {command: "reload_page"} };
			break;
		default:
...
function get_task_preparations( iTaskID, teTask )
{
	...
	*/
	...
	{
		set_error( 1, "Передан некорректный ID задачи", false )
		return oRes;
	}
 ...
 */
function GetProjects( iPersonID )
{
	return get_projects( iPersonID );
}

function get_projects( iPersonID, bGetOperations, arrStates )
{
	/*
...
function GetMyProjects(iPersonID, sTypeProjects) {
	...
	try {
		...
		set_error(oRes, 1, 'Отсутствует или не передан ID сотрудника.');
		return oRes;
	}
	...
	function getProjectPersonRoles(oProject, aRoles, iProjectID) {
		...
		if (sTypeProjects == 'open') {
			oProject.roles = 'Вы можете присоединиться к проекту';
	...
	if (ArrayCount(xarrProjects) > 0) {
		...
		for (project in xarrProjects) {
			...
			if (dProject != undefined) {
				...
				oProject.has_team = teProject.team_selected.HasValue ? tools_web.is_true(teProject.team_selected.Value) ? 'Да' : 'Нет' : 'Нет';
				...
				if ((sProjectStatus == 'active' && dEndDatePlan != '' && Date() > dEndDatePlan) || dEndDateFact > dEndDatePlan) {
					oProject.warning = 'Нарушен срок окончания';
				} 
...
function GetProjectFiles(iProjectID, bIncludeTaskFiles) {
	...
	try {
		...
		set_error(oRes, 1, 'Отсутствует или не передан ID проекта.');
		return oRes;
	}
...
function GetProjectParticipants(iProjectID) {
	...
	for (participant in xarrProjectParticipants) {
		...
		if (dParticipant != undefined) {
			...
			oParticipant.status = (teParticipant.is_excluded.HasValue && tools_web.is_true(teParticipant.is_excluded.Value)) ? 'Исключен из проекта' : 'В команде проекта';
...
function GetHierarchicalTasksPerson(iPersonID, iProjectID) {
	...
	function getTaskResult(id, task, executors, parent_id) {
		...
		oTaskRes.status = (task.status.HasValue) ? common.assessment_appraise_statuses.GetOptChildByKey(task.status.Value).name.Value : '';
		return oTaskRes;
	}
...
*/
function GetTheMostEffectiveManagers(iPersonID, iLimitProjects) {
	var oRes = tools.get_code_library_result_object();
	oRes.array = [];
	...
	try {
		...
		sChartTitle = 'Самые результативные руководители';
	}
...
function CreateProject(sCommand, iPersonID, iProjectID, SCOPE_WVARS, aFieldEntries) {
	...
	if (iPersonID == undefined || iPersonID == null || Trim(iPersonID) == '') {
		return close_form(oRes, 'Отсутствует или не передан ID сотрудника.');
	}
	...
	var sTitle = (isModeEdit) ? 'Изменение проекта' : 'Создание проекта';
	...
	if (isModeEdit && ArrayOptFirstElem(XQuery('for $elem in func_managers where MatchSome($elem/boss_type_id, ('+ sBossTypes +')) and $elem/person_id = '+ iPersonID +' return $elem/Fields("id")')) == undefined) {
		return close_form(oRes, 'Вы не являетесь руководителем одного из указанных типов.');
	}
	...
	if( ArrayOptFind( aIntersectFields, "This.id == 'start_date_plan'" ) != undefined && ArrayOptFind( aIntersectFields, "This.id == 'end_date_plan'" ) != undefined )
	{
		if( OptDate(get_form_field_val(aFormFields, 'start_date_plan') ) != undefined && OptDate(get_form_field_val(aFormFields, 'end_date_plan') ) != undefined )
		{
			...
			{
				return remote_action_alert(oRes, 'Планируемая дата начала не может быть больше чем планируемая дата завершения.');
			}
		}
	}
	if( ArrayOptFind( aIntersectFields, "This.id == 'start_date_fact'" ) != undefined && ArrayOptFind( aIntersectFields, "This.id == 'end_date_fact'" ) != undefined )
	{
		if( OptDate(get_form_field_val(aFormFields, 'start_date_fact') ) != undefined && OptDate(get_form_field_val(aFormFields, 'end_date_fact') ) != undefined )
		{
			...
			{
				return remote_action_alert(oRes, 'Фактическая дата начала не может быть больше чем фактическая дата завершения.');
			}
		}
	}
	...
	try {
		...
		return remote_action_alert(oRes, 'Не удалось открыть карточку существующего объекта. ('+ iProjectID +')');
	}
	...
	function save_fields_form() {
		...
		if (oResource != undefined) {
			...
			if (iResourceID != undefined) {
				...
				alert('[CreateProject]: Не удалось создать новый ресурс базы.')
			}
		}
		...
		function delete_project_manager(iProjectMenegerID) {
			try {
				...
				alert('[CreateProject]: Ошибка при удалении участника проекта с ID: ' + iProjectMenegerID);
			}
		}
	...
	function create_project() {
		switch(sCommand) {
			...
			{
				save_fields_form();
				return close_form(oRes, 'Проект успешно '+ (isModeEdit ? 'отредактирован' : 'создан') + '.', true);
			}
		}
	}
	...
	if (isModeEdit) {
		...
		if (!isPersonManager) {
			return close_form(oRes, 'Вы не можете редактировать данный проект, т.к. не являетесь его руководителем.');
		} else {
			create_project();
		}
	} else {
...
function CreateProjectParticipant(sCommand, iPersonID, tePerson, iProjectID, iProjectParticipantID, SCOPE_WVARS, aFieldEntries) {
	var oRes = tools.get_code_library_result_object();
	var aFields, isModeEdit;
	var sTitle = 'Добавить/редактировать участника';
	
	if (iPersonID == undefined || iPersonID == null || Trim(iPersonID) == '') {
		return close_form(oRes, 'Отсутствует ID сотрудника.');
	}
	...
	try {
		...
		return close_form(oRes, 'Отсутствует ID проекта.');
	}
	...
	if (iProjectParticipantID != undefined && iProjectParticipantID != null && Trim(iProjectParticipantID) != '') {
		...
		if (!isModeEdit) {
			return close_form(oRes, 'Указанный ID участника проекта отсутствует в проекте, заданном в параметре ID проекта.');
		} else {
			isModeEdit = true;
		}
	} else {
	...
	if (isModeEdit) {
		try {
			...
			return remote_action_alert(oRes, 'Не удалось открыть карточку участника проекта. ('+ iProjectParticipantID +')');
		}
	}
	...
	function save_fields_form() {
		...
		if (is_project_manager(iPPObjectID, iProjectID, iPPBossTypeID)) {
			return 'В проекте уже есть участник с выбранным сотрудником и типом руководителя.';
		}
		...
		if (!isModeEdit) {
			try {
				...
				return 'Не удалось создать участника проекта.';
			}
		...
		if (oResource != undefined) {
			...
			if (iResourceID != undefined) {
				...
				alert('[CreateProjectParticipant]: Не удалось создать новый ресурс базы.')
			}
		}
		...
		if (isModeEdit) {
			...
			if (bSendNotificationChangeRole && (ArrayCount(ArrayIntersect(aPPRolesBefore, aPPRolesAfter, 'This', 'This')) != ArrayCount(aPPRolesBefore))) {
				...
				if (!bSended) {
					alert('[CreateProjectParticipant]: Не удалось отправить уведомление о смене роли сотрудника в проекте.');
				}
			}
		} else {
		...
		switch(sCommand) {
			...
			case 'submit_form':
			{
				...
				if (!isModeEdit && bSendNotificationAfterInclude && tePP.status_id.HasValue && tePP.status_id.Value == 'active') {
					...
					if (!bSended) {
						alert('[CreateProjectParticipant]: Не удалось отправить уведомление о включении сотрудника в состав участников проекта.');
					}
				}
				var sTextDlgMessage = (isModeEdit) ? 'Информация об участнике проекта обновлена.' : 'Выбранный сотрудник добавлен в состав участников проекта.';
	...
	if (!is_project_manager(iPersonID, iProjectID)) {
		return close_form(oRes, 'Пользователь должен являться руководителем заданного проекта.');
	} else {
...
function JoinToProject(sCommand, iPersonID, iProjectID, SCOPE_WVARS, aFieldEntries) {
	var oRes = tools.get_code_library_result_object();
	var sTitle = 'Участвовать в проекте';
	var sProjectUrl = UrlSchema(CurRequest.Url) + '://' + CurRequest.UrlHost + tools_web.get_mode_clean_url(null, iProjectID);
	...
	if (cPP != undefined || cPR == undefined) {
		return close_form(oRes, 'Убедитесь, что вы не являетесь участником заданного проекта. Что проект имеет открытый тип вступления, находится в статусе "планируется" или "в работе" и имеет хотя бы одну роль.');
	}
	...
	function save_fields_form() {
		...
		try {
			...
			return remote_action_alert(oRes, 'Не удалось открыть документ проекта.');
		}
		...
		if (oResource != undefined) {
			...
			if (iResourceID != undefined) {
				...
				alert('[JoinToProject]: Не удалось создать новый ресурс базы.')
			}
		}
	...
	switch(sCommand) {
		...
		case 'submit_form':
			...
			var sText = 'Теперь вы участник выбранного проекта.';
			if (sStatePM == 'agreement') {
				sText = 'Заявка на участие в проекте отправлена.'
			}
			if (bSendNtfPM) {
				xarrPersonFMs = XQuery('for $elem in func_managers where $elem/catalog = "collaborator" and $elem/object_id = '+ iPersonID +' return $elem');
				sParamText = ArrayOptFirstElem(XQuery('for $elem in collaborators where $elem/id = '+ iPersonID +' return $elem/Fields("fullname")'), { fullname: 'Сотрудник' }).fullname + ';' + sProjectUrl;
				for (fm in xarrPersonFMs) {
					...
					if (!bSendNotification) {
						alert('[JoinToProject] Не удалось отправить уведомление руководителю сотрудника.');
					}
				}
			}
...
function ChangeProjectStatus(sCommand, iPersonID, iProjectID, sStatus) {
	...
	if (iPersonID == undefined || iPersonID == null || Trim(iPersonID) == '') {
		return remote_action_alert(oRes, 'Отсутствует ID сотрудника.');
	}
	
	if (iProjectID == undefined || iProjectID == null || Trim(iProjectID) == '') {
		return remote_action_alert(oRes, 'Отсутствует ID проекта.');
	}

	if (!is_project_manager(iPersonID, iProjectID)) {
		return remote_action_alert(oRes, 'Пользователь должен являться руководителем заданного проекта.');
	}
	...
	function set_poject_status(dProject) {
		...
		return {
			command: 'alert', 
			msg: 'Статус проекта успешно обновлен.',
	...
	function send_notification(iExecutor, sTextParam, iTaskID, teTask) {
		...
		if (!bSended) {
			alert('[ChangeProjectStatus] Не удалось отправить уведомление ответственному за исполнение задачи.')
		}
	}
	...
	function close_project(dProject) {
		...
		if (ArrayCount(xarrProjectTasks) > 0) {
			return {
				command: 'confirm', 
				msg: 'Внимание! В проекте есть незавершенные задачи. Завершить их автоматически?',
	...
	function cancel_project(dProject) {
		...
		if (ArrayCount(xarrProjectTasks) > 0) {
			return {
				command: 'confirm', 
				msg: 'Внимание! В проекте есть незавершенные задачи. Отменить их автоматически?',
	...
	dProject = tools.open_doc(iProjectID);
	if (dProject == undefined) {
		return remote_action_alert(oRes, 'Не удалось открыть карточку проекта.');
	}
	teProject = dProject.TopElem;
	...
	switch (sStatus) {
		case 'close':
		{
			if (teProject.allow_assessment.HasValue && teProject.allow_assessment.Value) {
				oRes.result = {
					command: 'confirm', 
					msg: 'Вы уверены, что хотите сменить статус проекта на "Завершен"?',
				...
				oRes.result = {
					command: 'confirm', 
					msg: 'Внимание! После завершения проекта вы уже не сможете оценить его участников! Вы точно хотите завершить проект?',
					confirm_result: close_project(dProject)
				};
				return oRes;
			}
		...
		case 'cancel':
		{
			oRes.result = {
				command: 'confirm', 
				msg: 'Вы уверены, что хотите сменить статус проекта на "Отменен"?',
		...
		case 'active':
		{
			oRes.result = {
				command: 'confirm', 
				msg: 'Вы уверены, что хотите сменить статус проекта на "В работе"?',
		...
		case 'plan':
		{
			oRes.result = {
				command: 'confirm', 
				msg: 'Вы уверены, что хотите сменить статус проекта на "Планируется"?',
				confirm_result: set_poject_status(dProject)
			};
			return oRes;
		}
...
function AddProjectParicipant(sCommand, iBossID, iPersonID, SCOPE_WVARS, aFieldEntries) {
	var oRes = tools.get_code_library_result_object();
	var sTitle = 'Добавить участника в проект';
	var iBossTypeID = get_boss_type_id('project_participant');
	var sProjectUrl = '';
	
	if (iBossID == undefined || iBossID == null || Trim(iBossID) == '') {
		return remote_action_alert(oRes, 'Отсутствует ID текущего пользователя (руководителя).');
	}
	...
	switch(sCommand) {
		...
		case 'submit_form':
			try {
				...
				return remote_action_alert(oRes, 'Сотрудник уже является участником заданного проекта.');
			}
			...
			if (bSendNotificationAfterInclude) {
				...
				if (!bSended) {
					alert('[AddProjectParicipant]: Не удалось отправить уведомление о включении сотрудника в состав участников проекта.');
				}
			}
		
			return close_form(oRes, 'Сотрудник был добавлен в проект.', true);
	}
}
...
function RemoveProjectRole(sCommand, iPersonID, iRoleID) {
	...
	if (iRoleID == undefined || iRoleID == null || Trim(iRoleID) == '') {
		return remote_action_alert(oRes, 'Отсутствует ID роли.');
	}
	...
	if (ArrayOptFirstElem(xarrPP) != undefined) {
		return remote_action_alert(oRes, 'В проекте есть участники с этой ролью, поэтому удалить ее нельзя.');
	}
	
	var docProjectParticipantRole = tools.open_doc( iRoleID );
	if( docProjectParticipantRole == undefined )
	{
		return remote_action_alert(oRes, 'Некорректный ID роли.');
	}
	if( docProjectParticipantRole.TopElem.project_id.HasValue )
	{
		...
		if( catProjectManagerBossType != undefined )
		{
			...
			{
				return remote_action_alert(oRes, 'У вас нет прав на удаление роли');
			}
		}
	}
	...
	function remove_role() {
		try {
			...
			return { command: 'alert', msg: 'Роль участников проекта была удалена.' }
		} catch(e) {
			return { command: 'alert', msg: 'Возникла ошибка в процессе удаления: ' + e }
		}
	}
	...
	if (sCommand == 'eval') {
		oRes.result = {
			command: 'confirm',
			msg: 'Вы уверены, что хотите удалить роль участников проекта?',
...
function RemoveProjectTask(sCommand, iPersonID, iTaskID) {
	...
	if (iTaskID == undefined || iTaskID == null || Trim(iTaskID) == '') {
		return remote_action_alert(oRes, 'Отсутствует ID задачи.');
	}
	...
	if (cTask != undefined && cTask.parent_task_id.HasValue) {
		...
		return remote_action_alert(oRes, 'Задача с ID: ' + iTaskID + ' отсутствует в базе.');
	}

	if (cTask.source_object_id.HasValue && !is_project_manager(iPersonID, cTask.source_object_id.Value)) {
		return remote_action_alert(oRes, 'Вы не являетесь руководителем проекта к которому относится задача.');
	}

	function remove_task() {
		var sMsg = (bStage) ? 'Этап проекта удален.' : 'Задача проекта удалена.';
		try {
			DeleteDoc(UrlFromDocID(iTaskID));
			return { command: 'alert', msg: sMsg }
		} catch(e) {
			sMsg = 'Возникла ошибка при удалении '+ (bStage ? 'этапа' : 'задачи') +' проекта. ';
	...
	switch (sCommand) {
		case 'eval':
		{
			var sMsg = 'Вы уверены, что хотите удалить '+ (bStage ? 'этап' : 'задачу') +' проекта?';
			...
			if (bTaskHasChilds) {
				return remote_action_alert(oRes, 'В данном этапе есть задачи, поэтому удалить его нельзя.');
			} else {
				oRes.result = {
					command: 'confirm',
					msg: sMsg,
...
function get_project_managers(iProjectID, iBossTypeID, bShortName) {
	...
	return [{ id: ArrayOptFirstElem(xarrPersons).id.Value, name: 'Отсутствуют' }];
}
...
function CreateProjectParticipantRole( sCommand, iPersonID, iProjectID, iProjectParticipantRoleID, SCOPE_WVARS )
{
	...
	{
		set_error( 1, "Передан некорректный ID текущего сотрудника", false );
		oRes.action_result = { command: "alert", msg: "Передан некорректный ID текущего сотрудника" };
		return oRes;
	}
	try
	{
	...
	{
		set_error( 1, "Передан некорректный ID проекта", false );
		oRes.action_result = { command: "alert", msg: "Передан некорректный ID проекта" };
		return oRes;
	}
	try
	{
	...
	{
		set_error( 1, "У сотрудника нет прав на создание/редактирование роли участников", false );
		oRes.action_result = { command: "alert", msg: "У сотрудника нет прав на создание/редактирование роли участников" };
		return oRes;
	}
	...
	if( iProjectParticipantRoleID != undefined )
	{
		...
		if( docProjectParticipantRole == undefined )
		{
			set_error( 1, "Передан некорректный ID роли участника", false );
			oRes.action_result = { command: "alert", msg: "Передан некорректный ID роли участника" };
			return oRes;
		}
	}
	...
	switch( sCommand )
	{
		case "eval":
			oRes.action_result = { 	command: "display_form",
						title: ( iProjectParticipantRoleID == undefined ? "Новая роль" : "Редактирование роли" ),
						header: ( iProjectParticipantRoleID == undefined ? "Новая роль" : "Редактирование роли" ),
						form_fields: [],
						buttons:
						[
							{ name: "submit", label: "Ok", type: "submit" },
							{ name: "cancel", label: "Отменить", type: "cancel"},
							{ name: "submit", label: "Редактировать уровни знаний", type: "submit", submit_type: "edit_knowledge"}
						],
						no_buttons: false						
					};
			docProject = tools.open_doc( iProjectID )
			oRes.action_result.form_fields.push( { name: "project_name_paragraph", type: "paragraph", value: "Проект" + " - " + docProject.TopElem.name } );
			oRes.action_result.form_fields.push( { name: "name", label: "Название", type: "string", value: ( docProjectParticipantRole != null ? docProjectParticipantRole.TopElem.name.Value : "" ), mandatory: true } );
			oRes.action_result.form_fields.push( { name: "participant_num", label: "Количество участников", type: "integer", value: ( docProjectParticipantRole != null ? docProjectParticipantRole.TopElem.participant_num.Value : "" ), mandatory: false } );
			oRes.action_result.form_fields.push( { name: "knowledge_parts_value", label: "Требуемые знания", type: "foreign_elem", catalog: "knowledge_part", multiple: true, value: ( docProjectParticipantRole != null ? ArrayMerge( docProjectParticipantRole.TopElem.knowledge_parts, "This.knowledge_part_id.Value", ";" ) : "" ), display_value: ( docProjectParticipantRole != null ? ArrayMerge( docProjectParticipantRole.TopElem.knowledge_parts, "This.knowledge_part_name.Value", "|||" ) : "" ), mandatory: false } );
			oRes.action_result.form_fields.push( { name: "comment", label: "Комментарий", type: "text", value: ( docProjectParticipantRole != null ? docProjectParticipantRole.TopElem.comment.Value : "" ), mandatory: false } );
			break;
		case "submit_form":
			oFormFields = null;
			var form_fields = SCOPE_WVARS.GetOptProperty( "form_fields", "" )
			...
			{
				oRes.action_result = { 	command: "display_form",
						title: "Редактирование знаний" ,
						header: "Редактирование знаний",
						form_fields: [],					
					};
				oRes.action_result.form_fields.push( { name: "name", label: "Название", type: "hidden", value: docProjectParticipantRole.TopElem.name.Value, mandatory: true } );
				oRes.action_result.form_fields.push( { name: "participant_num", label: "Количество участников", type: "hidden", value: docProjectParticipantRole.TopElem.participant_num.Value , mandatory: false } );
				oRes.action_result.form_fields.push( { name: "knowledge_parts_value", label: "Требуемые знания", type: "hidden", value: ArrayMerge( docProjectParticipantRole.TopElem.knowledge_parts, "This.knowledge_part_id.Value", ";" ), mandatory: true } );
				oRes.action_result.form_fields.push( { name: "comment", label: "Комментарий", type: "hidden", value: docProjectParticipantRole.TopElem.comment.Value, mandatory: true } );
			...
			oRes.action_result = { command: "close_form", msg: "Роль участника сохранена.", confirm_result: { command: "reload_page" } };
			break;
...
function get_project_participant_context( iProjectParticipantID, teProjectParticipant )
{
	...
	if( ArrayOptFirstElem( teProjectParticipant.participant_roles_id ) != undefined )
	{
		...
		if( ArrayOptFirstElem( xarrProjectParticipantRoles ) != undefined )
		{
			oRes.context.sTitleRoles = ArrayCount( xarrProjectParticipantRoles ) == 1 ? "Требования к роли" : "Требования к ролям";
...
*/
function GetProjectsContext( iPersonID )
{
	return get_projects_context( iPersonID );
}
function get_projects_context( iPersonID )
{
	var oRes = tools.get_code_library_result_object();
	oRes.context = {
		"projects_num": 0,
	...
	{
		xarrProjectParticipants = XQuery( "for $elem in project_participants where $elem/boss_type_id = " + catProjectManagerBossType.id + " and ( $elem/status_id = 'active' or $elem/status_id = null() )  return $elem" );
		xarrProjectParticipants = ArrayIntersect( xarrProjectParticipants, aSubordinateIDs, "This.object_id", "This" );
	}
	else
	{
		xarrProjectParticipants = XQuery( "for $elem in project_participants where $elem/object_id = " + iPersonID + " and $elem/boss_type_id = " + catProjectManagerBossType.id + " and ( $elem/status_id = 'active' or $elem/status_id = null() )  return $elem" );
	}
	...
	xarrProjects = XQuery( "for $elem in projects where MatchSome( $elem/id, ( " + ArrayMerge( ArraySelectDistinct( xarrProjectParticipants, "This.project_id" ), "This.project_id", "," ) + " ) ) and $elem/status != 'cancel' return $elem" );
	if( ArrayOptFirstElem( xarrProjects ) == undefined )
	{
		return oRes;
	}
	oRes.context.projects_num = ArrayCount( xarrProjects );
	oRes.context.active_projects_num = ArrayCount( ArraySelect( xarrProjects, "This.status == 'active'" ) );
	...
	oRes.context.unexpired_projects_num = ArrayCount( ArraySelect( xarrProjects, "This.status == 'close' && This.end_date_fact.HasValue && This.end_date_fact <= This.end_date_plan" ) );
	
	xarrTasks = XQuery( "for $elem in tasks where MatchSome( $elem/source_object_id, ( " + ArrayMerge( xarrProjects, "This.id", "," ) + " ) ) and $elem/status = '1' return $elem" );
...
function EditProjectParticipantKnowledge( sCommand, iPersonID, iProjectParticipantID, SCOPE_WVARS )
{
	...
	{
		set_error( 1, "Передан некорректный ID участника проекта", false );
		oRes.action_result = { command: "alert", msg: "Передан некорректный ID участника проекта" };
		return oRes;
	}
	...
	{
		set_error( 1, "Нельзя оценивать участника до включения его в мероприятие", false );
		oRes.action_result = { command: "alert", msg: "Нельзя оценивать участника до включения его в мероприятие" };
		return oRes;
	}
	...
	{
		set_error( 1, "Не указан проект у участника", false );
		oRes.action_result = { command: "alert", msg: "Не указан проект у участника" };
		return oRes;
	}
	...
	if( !bAccess )
	{
		set_error( 1, "У вас нет прав на оценку участника проекта", false );
		oRes.action_result = { command: "alert", msg: "У вас нет прав на оценку участника проекта" };
		return oRes;
	}
	...
	switch( sCommand )
	{
		case "eval":
			oRes.action_result = { 	command: "display_form",
						title: ( "Оценка участника " + docProjectParticipant.TopElem.object_name ),
						header: ( "Оценка участника " + docProjectParticipant.TopElem.object_name ),
						form_fields: [],					
					};
			oRes.action_result.form_fields.push( { name: "form_title", type: "paragraph", value: "Проект" + " : " + catProject.name } );
			oRes.action_result.form_fields.push( { name: "skills_title", type: "paragraph", value: "Навыки участника" } );
			...
			for( _kp in arrKnowledges )
			{
				...
				field = { name: "current_level_id_" + _kp.knowledge_part_id, label: "Уровень", type: "select", value: "", entries: [] };
				field.entries.push( { name: "Не оценено", value: "___no_assessment___" } );
				for( _level in _kp.levels )
				{
					field.entries.push( { name: RValue( _level.title ), value: RValue( _level.value ) } );
				}
				oRes.action_result.form_fields.push( field );
				oRes.action_result.form_fields.push( { name: "comment_" + _kp.knowledge_part_id, label: "Комментарий", type: "string", value: "" } );
			}
		...
		case "submit_form":
			...
			oRes.action_result = { command: "close_form", msg: "Оценки участнику проекта выставлены.", confirm_result: { command: "reload_page" } };
			break;
...
function CreateUpdateProjectTask( sCommand, iPersonID, iProjectID, iTaskID, SCOPE_WVARS )
{
	...
	function add_field( field_name )
	{
		switch( field_name )
		{
			case "source_object_id":
				oRes.action_result.form_fields.push( { name: field_name, label: "Проект", type: "foreign_elem", catalog: "project", value: iProjectID, display_value: docProject.TopElem.name.Value, mandatory: true, disabled: true } );
				break;
			case "name":
				oRes.action_result.form_fields.push( { name: field_name, label: "Название", type: "string", value: ( docTask != null ? docTask.TopElem.name.Value : "" ), mandatory: true, disabled: check_disabled( field_name ) } );
				break;
			case "plan":
				oRes.action_result.form_fields.push( { name: field_name, label: "Описание", type: "text", value: ( docTask != null ? docTask.TopElem.plan.Value : "" ), mandatory: true, disabled: check_disabled( field_name ) } );
				break;
			case "fact":
				oRes.action_result.form_fields.push( { name: field_name, label: "Решение", type: "text", value: ( docTask != null ? docTask.TopElem.fact.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "date_plan":
				oRes.action_result.form_fields.push( { name: field_name, label: "Плановая дата начала", type: "date", value: ( docTask != null ? docTask.TopElem.date_plan.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "end_date_plan":
				oRes.action_result.form_fields.push( { name: field_name, label: "Плановая дата завершения", type: "date", value: ( docTask != null ? docTask.TopElem.end_date_plan.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "executor_id":
				field = { name: field_name, label: "Исполнитель", type: "select", value: ( docTask != null ? docTask.TopElem.executor_id.Value : "" ), entries: [], disabled: check_disabled( field_name ) };
				...
				break;
			case "plan_labor_costs":
				oRes.action_result.form_fields.push( { name: field_name, label: "Плановые трудозатраты", type: "integer", value: ( docTask != null ? docTask.TopElem.plan_labor_costs.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "status":
				field = { name: field_name, label: "Статус", type: "select", value: ( docTask != null ? docTask.TopElem.status.Value : "" ), entries: [], disabled: check_disabled( field_name ) };
				for( _state in common.assessment_appraise_statuses )
				{
				...
				break;
			case "date_fact":
				oRes.action_result.form_fields.push( { name: field_name, label: "Фактическая дата начала", type: "date", value: ( docTask != null ? docTask.TopElem.date_fact.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "end_date_fact":
				oRes.action_result.form_fields.push( { name: field_name, label: "Фактическая дата завершения", type: "date", value: ( docTask != null ? docTask.TopElem.end_date_fact.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "fact_labor_costs":
				oRes.action_result.form_fields.push( { name: field_name, label: "Фактические трудозатраты", type: "integer", value: ( docTask != null ? docTask.TopElem.fact_labor_costs.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
			case "value":
				oRes.action_result.form_fields.push( { name: field_name, label: "Оценка", type: "integer", value: ( docTask != null ? docTask.TopElem.value.Value : "" ), mandatory: false, disabled: check_disabled( field_name ) } );
				break;
		}
	}
	...
	{
		set_error( 1, "У сотрудника нет прав на создание задачи", false );
		oRes.action_result = { command: "alert", msg: "У сотрудника нет прав на создание задачи" };
		return oRes;
	}
	...
	if( iTaskID != undefined )
	{
		...
		{
			set_error( 1, "Передан некорректный ID задачи", false );
			oRes.action_result = { command: "alert", msg: "Передан некорректный ID задачи" };
			return oRes;
		}
	}
	...
	switch( sCommand )
	{
		case "eval":
			oRes.action_result = { 	command: "display_form",
						title: ( iTaskID == undefined ? "Новая задача" : "Редактирование задачи" ),
						header: ( iTaskID == undefined ? "Новая задача" : "Редактирование задачи" ),
						form_fields: [],				
					};
			for( _field in arrFields )
			{
