...
*/
function HandleEventFinishCourseTest(objectDoc, learningDoc)
{
	try 
	{
		if (objectDoc == undefined || objectDoc == null || objectDoc == '') 
			throw '';
	}
	catch(e) 
	{
	...
	if (objectDoc != null && learningDoc != null) 
	{
		...
		if (ArrayOptFirstElem(xarrCareerReserve) != undefined)
		{
			for (catCareerReserve in xarrCareerReserve)
			{
				...
				taskCourse = ArrayOptFind(teCareerReserve.tasks, "This.object_type == obj_type && This.object_id == iObjectID");
				
				if ( taskCourse != undefined)
				{
					if (learningDoc.state_id == 4)
						taskCourse.status = 'passed';
					else if (learningDoc.state_id == 3)
						taskCourse.status = 'failed';
					
					if (taskCourse.parent_task_id != undefined)
					{
