...
function cast_KeyPosition(teKeyPosition)
{
	...
	oItem.text_html += "<p class=\"risk_levels\"><span class=\"title\">Факторы риска:</span><span class=\"text\">" + oItem.risk_levels + "</span></p>\n";
...
function GetKeyPositions( iPersonID, sTypeCollaborator, arrBossTypesID, bReturnByBoss, bReturnByHR, iAppLevel, sState, sFilter, oCollectionParams )
{
	...
	if ( iAppLevel == null || iAppLevel == undefined )
	{
		if ( tools_web.is_true( bReturnByBoss ) )
		{
			...
			try
			{
				arrPositionIDs = ArrayUnion( arrPositionIDs, ArrayExtract( tools.call_code_library_method( "libMain", "get_user_collaborators", [ iPersonID, sTypeCollaborator, null, null, null, arrBossTypesID ] ), "This.position_id.Value"));
			}
			catch( ex )
			{
				oRes.error = 1;
		...
		if ( tools_web.is_true( bReturnByHR ) )
		{
			...
			try
			{
				arrPositionIDs = ArrayUnion( arrPositionIDs, ArrayExtract( tools.call_code_library_method( "libMain", "get_user_collaborators", [ iPersonID, "func_subordinates", null, "", null, arrHRBossTypesID ] ), "This.position_id.Value" ) );
			}
			catch( ex )
			{
				oRes.error = 1;
	...
	if ( sXQConds == '' )
	{
		var sReqLongListKeyPosition = "for $elem_qc in key_positions where MatchSome( $elem_qc/position_id,(" + ArrayMerge( arrPositionIDs, 'This', ',' ) + " ) ) return $elem_qc ";
		var xarrKeyPositions = tools.xquery( sReqLongListKeyPosition );
	}
	else
	{
		var xarrKeyPositions = tools.xquery( "for $elem in key_positions " + sXQConds + " order by $elem/id return $elem" );
	}
...
function GetSuccessors( iPersonID, sTypeCollaborator, arrBossTypesID, bReturnByBoss, bReturnByHR, iAppLevel, sState, sFilter, oCollectionParams )
{
	var oRes = tools.get_code_library_result_object();
	oRes.array = [];
	...
	if ( iAppLevel == null || iAppLevel == undefined )
	{
		if ( tools_web.is_true( bReturnByBoss ) )
		{
			...
			try
			{
				arrPersonIDs = ArrayUnion(arrPersonIDs, ArrayExtract(tools.call_code_library_method( "libMain", "get_user_collaborators", [ iPersonID, sTypeCollaborator, null, null, null, arrBossTypesID ] ), "This.id.Value"));
			}
			catch(err)
			{
				oRes.error = 1;
		...
		if ( tools_web.is_true( bReturnByHR ) )
		{
			...
			try
			{
				arrPersonIDs = ArrayUnion(arrPersonIDs, ArrayExtract(tools.call_code_library_method( "libMain", "get_user_collaborators", [ iPersonID, "func_subordinates", null, "", null, arrHRBossTypesID ] ), "This.id.Value"));
			}
			catch(err)
			{
				oRes.error = 1;
...
function AddChangeKeyPosition(sFormCommand, sFormFields, iKeyPositionIDParam, bSendBossNotification, iCurUserID, iCurCareerReserveTypeID, sStateCreate, iAppLevel, iManagerTypeID )
{
	...
	{	
		...
		{
			oRet.error = 503;
			oRet.errorText = "Dозможность создания ключевых должностей отключена в настройках.";
			oRet.result = close_form(oRet.errorText);
			return oRet;
		}
		...
		if ( sFormCommand == "eval" || ( sFormCommand == "submit_form" && sNextStep == "form" ) )
		{
			...
			{
				arrFormFields.push({name: "paragraph_position_id", type: "paragraph", value: "<b>Должность:</b>&nbsp;" + teKeyPosition.person_position_name.Value}); 
			}
			...
			if(!bIsReadOnly)
			{
				...
				//arrFormFields.push({name: "risk_perspective_id", label: "Уровень риска", title: "Выберите элемент", type: "foreign_elem", multiple: false, catalog: "risk_perspective", display_value: sRiskPerspectiveName, value: (teKeyPosition != null ? teKeyPosition.risk_perspective_id.Value : null), column : 2}); 
				...
				arrFormFields.push({name: "risk_perspective_id", label: "Уровень риска", type: "select", entries: arrRiskPerspectiveEntrys, value: (teKeyPosition != null ? teKeyPosition.risk_perspective_id.Value : null)}); 
				...
				arrFormFields.push({name: "risk_levels", label: "Факторы риска", type: "list", entries: arrRiskLevelsEntrys, value: (teKeyPosition != null ? ArrayExtract(teKeyPosition.risk_levels, "This.risk_level_id.Value") : [])}); 

			}
			else
			{
				arrFormFields.push({name: "paragraph_risk_perspective_id", type: "paragraph", value: "<b>Уровень риска:</b>&nbsp;" + sRiskPerspectiveName }); 
				...
				arrFormFields.push({name: "paragraph_risk_levels", type: "paragraph", value: (teKeyPosition != null ? "<b>Факторы риска:</b><br>" + ArrayMerge(teKeyPosition.risk_levels, "This.risk_level_id.ForeignElem.name.Value", ", ") : "") }); 
			}
			...
			var sTitle = teKeyPosition != null ? "Редактировать" : "Добавить";
			var arrButtons = [];
			var oForm = {
...
function AddChangeSuccessor(sFormCommand, sFormFields, iSuccessorIDParam, bSendPersonNotificationOnCreate, bSendHRNotificationOnChange, iCurUserID, iCurKeyPositionID, bSendOwnerNotificationOnChangeCreate, iPersonNotificationID, iHRNotificationID, iOwnerNotificationIDOnCreate, iOwnerNotificationIDOnChange, bCheckEfficiencyCollaboratorParam, bCheckEfficiencyCanChangeParam, bCheckPotentialCollaboratorParam, bCheckPotentialCanChangeParam, iAppLevel, iManagerTypeID )
{
	...
	{	
		...
		else if ( sFormCommand == "submit_form" && sNextStep == "do" )
		{
			...
			oRet.result = set_successor( docSuccessor, null, oFormField, oParam );
		}
	}
	catch(err)
	{
 ...
 * @param {string} sObjectCode - Код фактора риска (code)
 * @param {string} sObjectName - Название фактора риска (name)
 * @returns {WTLPETextResult}
*/
function ChangeCreateRiskLevel(iObjectID, sObjectCode, sObjectName){
	var oRes = tools.get_code_library_result_object();
	oRes.result = "";
	...
	{
		oRes.error = 501;
		oRes.errorText = "{ text: 'Invalid param iObjectID.', param_name: 'iObjectID' }";
		return oRes;
	}
	
	if(IsArray(sObjectCode))
	{
		oRes.error = 501;
		oRes.errorText = "Аргумент функции 'sObjectCode' является массивом";
		return oRes;
	}
	
	if(IsArray(sObjectName))
	{
		oRes.error = 501;
		oRes.errorText = "Аргумент функции 'sObjectName' является массивом";
		return oRes;
	}
	
	iObjectID = OptInt(iObjectID);
	sObjectCode = Trim(String(sObjectCode));
	sObjectName = Trim(String(sObjectName));

	if(iObjectID == undefined){ // create risk level
		docRiskLevel = tools.new_doc_by_name("risk_level", false);
		docRiskLevel.BindToDb();
		docRiskLevel.TopElem.code = sObjectCode;
		docRiskLevel.TopElem.name = sObjectName;
		docRiskLevel.Save();

		oRes.result = "Фактор риска успешно создан"
	} else { // edit risk level
		...
		docRiskLevelTE.code.Value = sObjectCode
		docRiskLevelTE.name.Value = sObjectName
		docRiskLevel.Save();
...
function set_key_position( docKeyPosition, oSetParams, bSendBossNotification, iPersonID, sStateCreate )
{
	...
	if(oSetParams.HasProperty("file"))
	{
		...
		if( oFile.url != "" )
		{
			...
			bDoSave = true;
		} else if(oFile.value != undefined || oFile.value != null)
		{
			teKeyPosition.files.ObtainChildByKey(OptInt(oFile.value));
		}
	}
...
function set_successor(docSuccessor, docKeyPosition, oFormField, oAddParam)
{
	...
	if ( docSuccessor == null )
	{
		...
		{
			return close_form("Выбранный вами сотрудник уже является преемником на эту ключевую должность", true); 
			/*
	...
	if(oFormField.HasProperty("file"))
	{
		...
		{
			teSuccessor.files.ObtainChildByKey(OptInt(oFile.value));
		}
	}
...
//toLog(teSuccessor.Xml )	
	...
	if ( bDoSave )
	{
		...
		return close_form((bIsNew ? "Для ключевой должности добавлен преемник" : "Информация о преемнике изменена"), true);
	}
	else return close_form(null, false);
