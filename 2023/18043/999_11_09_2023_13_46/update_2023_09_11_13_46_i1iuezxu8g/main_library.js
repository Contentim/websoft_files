...
*/
/**
 * @typedef {Object} WTLPEBoolResult
 * @property {integer} error – код ошибки
 * @property {string} errorText – текст ошибки
 * @property {boolean} flag – возвращаемый признак
*/
	...
	try
	{
		...
		for(itemLector in xqLectors)
		{
			...
			if ( teLector.type.HasValue )
				...
				{
					case "invitee":
						oLector.type = "Внешний";
						break;
					case "collaborator":
						oLector.type = "Внутренний"
						break;
				}
...
function GetPersonCollaborators( iPersonID, sTypeCollaborator, iMaxCnt, bShowDismiss, sSearch, bAllHier, arrBossTypesID, oCollectionParams )
{
	...
	var arrCareerReserves = ArraySelectAll(tools.xquery("for $elem in career_reserves where $elem/status='active' and $elem/position_type='adaptation' order by $elem/person_id return $elem/Fields('id','person_id')"));
	var arrSuccessors = ArraySelectAll(tools.xquery("for $elem in successors where $elem/status='active' or $elem/status='approved' order by $elem/person_id return $elem/Fields('id','person_id')"));
	var arrPersonReserves = ArraySelectAll(tools.xquery("for $elem in personnel_reserves where $elem/status='in_reserve' order by $elem/person_id return $elem/Fields('id','person_id','career_reserve_type_id')"));
	var arrFuncManagers = ArraySelectAll(tools.xquery("for $elem in func_managers order by $elem/person_id return $elem/Fields('id','person_id')"));
	var arrExperts = ArraySelectAll(tools.xquery("for $elem in experts order by $elem/person_id return $elem/Fields('id','person_id')"));
	var arrTutors = ArraySelectAll(tools.xquery("for $elem in tutors order by $elem/person_id return $elem/Fields('id','person_id')"));
	var xarrAllAssessmentForms = ArraySelectAll(tools.xquery("for $elem in pas where $elem/assessment_appraise_type='activity_appraisal' and $elem/is_done = true() and some $appr in assessment_appraises satisfies ($elem/assessment_appraise_id = $appr/id and $appr/status = '1' and($appr/end_date > " + XQueryLiteral(DateOffset(Date(), (0-iEffectivenessPeriod)*86400)) + " or $appr/end_date = null())) order by $elem/person_id ascending, $elem/modification_date descending return $elem/Fields('id','person_id','overall')"));
	...
	for( _col in arrCollaborators )
	{
		...
		obj.efficiency_estimation_id = (_col.efficiency_estimation_id.HasValue ? _col.efficiency_estimation_id.Value : null);
		obj.efficiency_estimation = (_col.efficiency_estimation_id.OptForeignElem != undefined ? _col.efficiency_estimation_id.OptForeignElem.name : '');
		
		// Оценка потенциала
		obj.development_potential_id = (_col.development_potential_id.HasValue ? _col.development_potential_id.Value : null);
		obj.development_potential = (_col.development_potential_id.OptForeignElem != undefined ? _col.development_potential_id.OptForeignElem.name : '');
 ...
 * @author BG
*/
function get_user_collaborators( iUserID, sTypeCollaborator, bShowDismiss, sSearch, bAllHier, arrBossTypesID, oCollectionParams, bWithoutUseManagementObject )
{
	...
	var oGetSubordinateParams = {
			...
			arrFieldsNames: ['id','fullname','position_name','email','mobile_phone','org_name','position_parent_name','hire_date','sex','birth_date','is_dismiss','current_state'],
		...
		for(oFilter in arrFilters)
		{
			...
			else if(oFilter.type == 'select')
			{
				switch(oFilter.id)
				{
					case "f_sex":
					{
						xqConds.push( "MatchSome( $i/sex, ( " + ArrayMerge(oFilter.value, "XQueryLiteral(This.value)", ",") + " ) )" );
						XQSubordinateConds.push( "MatchSome( $elem/sex, ( " + ArrayMerge(oFilter.value, "XQueryLiteral(This.value)", ",") + " ) )" );
						arrTmpXQCond = [];
					...
					case "f_status":
					{
						...
						XQSubordinateConds.push( "( " + StrReplace(ArrayMerge(arrTmpXQCond, "This", " or "), "$i/", "$elem/") + " )" )
			...
			else
			{
				...
				switch(oFilter.id)
				{
					...
					case "fullname":
					{
						...
						{
							xqConds.push( "contains($i/fullname, " + XQueryLiteral(oFilter.value) + " )" );
	...
	if(bArrIDsIsNull)
	{
		...
		var sCollaboratorReq = "for $i in collaborators where " + ArrayMerge( xqConds, "This", " and " ) + StrReplace(sCondSort, "[{CURSOR}]", "$i") + " return $i/Fields('id','fullname','position_name','email','mobile_phone','phone','org_name','position_parent_name','hire_date','sex','birth_date','is_dismiss','current_state','development_potential_id','efficiency_estimation_id')";
	...
	switch( sCommand )
	{
		case "eval":
		{
			...
			if ( teRequestType.object_type.HasValue )
			{
				...
				{
					arrFormFields.push( { name: "selected_object_id", label: teRequestType.object_type.ForeignElem.title.Value, type: "foreign_elem", catalog_name: teRequestType.object_type.Value, query_qual: teRequestType.object_query_qual.Value, value: iObjectID,  mandatory: true, validation: "nonempty" } );
					bDoStep1 = true;
				}
		...
		case "submit_form":
			...
			if ( fldCustomElems != null && !bCheckCustomFields )
			{
				...
				if( ArrayOptFirstElem( fldCustomElemsFields ) != undefined )
				{
					oRes.action_result = { 	command: "display_form",
							title: requestTypeDoc.name.Value,
							header: "Заполните дополнительные поля",
							form_fields: arrFormFields };
			...
			oRes.action_result = { command: "close_form", msg: ( requestTypeDoc.create_message.HasValue ? requestTypeDoc.create_message.Value : "Заявка успешно создана" ), confirm_result: {command: "reload_page"} };
			break;
		default:
...
function CreateRequestMain( iRequestTypeID, teRequestType, oSourceParam, iPersonID, tePerson, docNewRequest )
{
	...
	if ( requestDoc.TopElem.type.HasValue )
	{
		...
		{
			oRes.error = 1;
			oRes.errorText = "Не выбран объект";
			return oRes;
		}
		else if( iPersonID != null )
		{
			if ( ! global_settings.settings.recruitment.default_request_type_id.HasValue || iRequestTypeID != global_settings.settings.recruitment.default_request_type_id )
			{
				...
				{
					oRes.error = 1;
					oRes.errorText = "У вас уже есть заявка по этому объекту.";
					return oRes;
				}
			}
		}
...
function get_request_group_persons(iRequestID, bHasDismissed, oCollectionParams)
{
	...
	{
		oRes.error = 501
		oRes.errorext = "Не передан ID заявки"
	}
	...
	{
		oRes.error = 501
		oRes.errorext = StrReplace("Не найдена заявка по указанному ID: [{PARAM1}]", "{PARAM1}", iRequestID);
	}
	...
	{
		oRes.error = 501
		oRes.errorext = StrReplace("Переданный ID не является ID заявки: [{PARAM1}]", "{PARAM1}", teRequest.Name);
	}
...
function _ruleCheck(_file, _person_id, _tePerson, _fileInfo, _config, _ores)
{
	...
	if (_file.FileName != "")
	{
		...
		if (_config != null)
		{
			...
			{
				_ores.error = 6; _ores.errorText = "Превышен допустимый размер файла"; return false;
			}
...
function JoinChunk( guid, file_name, fileSize, countChunk, iPersonID, tePerson ) 
{
	...
	try 
	{
		...
		{
			oResp.error = 1;
			oResp.message = "Неизвестный формат файла.";
			return oResp;
		}
...
function UploadFile( Request, bJoinFile, iPersonID, tePerson ) 
{
	var sFileChunkFolderUrl = "x-local://wt_data/attachments/filechunk";
	try
	{
		...
		if (fileName != undefined && Trim(fileName) != '') 
		{
			...
			if (oResp.error == 0) {
				if ( numberChunk == countChunk - 1 ) 
				{
					if( bJoinFile )
					{
						var oResJoin = JoinChunk(guid, fileName, fileSize, countChunk, iPersonID, tePerson);
						if (oResJoin.Success) 
						{
...
//		return oRes;
	...
	if( oResUpload.chunk )
	{
		...
		{
			oRes.error = 1;
			oRes.errorText = "Неизвестный формат файла.";
			return oRes;
		}
...
function get_socket_url( sPortalUrl )
{
	...
	if( t_info.GetOptProperty( "WebsocketQueue" ) != undefined )
	{
		if( t_info.WebsocketQueue.GetOptProperty( "Urls" ) != undefined )
		{
			...
			{
				sUrl = ArrayOptFind( t_info.WebsocketQueue.Urls, "StrBegins( This, 'wss://' ) || StrBegins( This, 'ws://' )" );
			}
			if( sUrl != undefined )
	...
	if( t_info.GetOptProperty( "MediaService" ) != undefined )
	{
		if( t_info.MediaService.GetOptProperty( "Urls" ) != undefined )
		{
			if(  sPortalUrl != "" )
			{
				sUrl = ArrayOptFind( t_info.MediaService.Urls, "StrBegins( This, " + XQueryLiteral( String( ( UrlSchema( sPortalUrl ) == "https" ? "wss://" : "ws://" ) ) ) + " )" );
			}
			else
			{
				sUrl = ArrayOptFind( t_info.MediaService.Urls, "StrBegins( This, 'wss://' ) || StrBegins( This, 'ws://' )" );
			}
			if( sUrl != undefined )
			{
				oRes.ws_media_service_url = sUrl;
			}
			if( StrContains( oRes.ws_media_service_url, "*" ) && sPortalUrl != "" )
			{
				oRes.ws_media_service_url = StrReplace( oRes.ws_media_service_url, "*", String( UrlHost( sPortalUrl ) ).split( ":" )[ 0 ] );
			}
		}
	}
				...
				switch(itemCTField.type.Value)
				{
					...
					case "file":
					{
						...
						if( iFieldObjectID != undefined )
						{
							...
							{
								oCustomElem.value = RValue( tools.get_disp_name_value( docFieldObject.TopElem ) );
							}
							else
							{
...
function AddResponse(iResponseTypeIDParam, iPersonID, arrFormFields, oAddParam)
{
	...
	{
		oRet.error = 501;
		oRet.errorText = "Не указан тип отзыва";
		return oRet;
	}
	...
	{
		oRet.error = 200
		oRet.errorText = "0::" + tools_web.get_web_const( 'r4ycn3hwme', curLngWeb );
		return oRet;
	}
	...
	oRet.paragraph = "Отзыв успешно создан";
	return oRet;
}
 ...
 * @param {string} sCurrency - Валюта, в которой выражается благодарность. "Спасибо" по умолчанию.
 * @param {boolean} bScoresWithoutDebiting - Только начислять баллы, без списания. Для валюты "Спасибо" не учитывается
 * @returns {WTLPETextResult}
*/
function GivePersonFeedback(iPersonID, iPoints, iCompetence, sComment, sCurrency, bScoresWithoutDebiting )
{
	...
	{
		oRet.error = 1;
		oRet.errorText = "Не указан сотрудник, получающий благодарность";
		return oRet;
	}
	...
	{
		oRet.error = 1;
		oRet.errorText = "Не указан размер благодарности";
		return oRet;
	}
	...
	{
		oRet.errorText = "Ваша обратная связь сохранена.";
		oRet.paragraph = "close_form";
	}
	else
	{
		oRet.error = 1;
		oRet.errorText = (sType != "thanks" && !doBalance) ? "Нехватка баллов на балансе!" : "Не удалось передать баллы." ;
		oRet.paragraph = "alert";
	}
...
function lp_person_feedback(sCommand, sFormFields, iPersonID, iAmntPoints, sCompetence, sText, sCurrency, iCompetenceProfileID, curUserID, bScoresWithoutDebiting)
{
	...
	if ( sCommand == "submit_form" )
	{
		...
		if ( oFormFields != undefined )
		{
			...
			{
				if(bBalanceCurUserGreaterZero && bBalanceCurUserGreaterBalancePoints){
					sMsg = "Нехватка баллов на балансе!"
				} else {
...
function GetBossSubordinateContext( iPersonID, arrCaclulateParam, dPeriodStart, dPeriodEnd, PersonnelTurnoverPeriod, bWithoutFuncManagers )
{
	...
	if(arrCaclulateParam.indexOf("experience" ) >= 0)
	{
	...
	if(arrCaclulateParam.indexOf("percent_womans" ) >= 0)
	{
	...
	{
		var sReqPDPForms = "for $elem in pas where $elem/is_done = true() and $elem/assessment_appraise_type = 'development_plan' and some $aa in assessment_appraises satisfies ($elem/assessment_appraise_id = $aa/id and $aa/end_date >= " + XQueryLiteral( DateOffset( Date(), ( 0 - ( iEffectivenessPeriod * 3 ) ) * 86400 ) ) + ") return $elem/Fields('id','person_id')";
	...
	if(arrCaclulateParam.indexOf("percent_bosses" ) >= 0)
	{
		...
		{
			var sBossReq = "for $elem_qc in func_managers where MatchSome($elem_qc/person_id, (" + sSubordinateIDs + ")) and $elem_qc/catalog = 'position' return $elem_qc/Fields('person_id')"
		}
		else
		{
			var sBossReq = "for $elem_qc in func_managers where MatchSome($elem_qc/person_id, (" + sSubordinateIDs + ")) and ($elem_qc/catalog = 'collaborator' or $elem_qc/catalog = 'org' or $elem_qc/catalog = 'subdivision' or $elem_qc/catalog = 'position') return $elem_qc/Fields('person_id')"
		}
		
		var arrBosses = tools.xquery(sBossReq);
		arrBosses = tools.xquery("for $elem_qc in collaborators where MatchSome($elem_qc/id, (" + ArrayMerge(arrBosses, "person_id", ",") + ")) return $elem_qc/Fields('id')")
	...
	{
		var arrVacacies = tools.xquery("for $elem_qc in interval_schedules where MatchSome($elem_qc/person_id, (" + sSubordinateIDs + ")) return $elem_qc");
	...
	if(arrCaclulateParam.indexOf("percent_turnover" ) >= 0)
	{
		...
		arrAllSubIDs = tools.xquery( "for $elem_qc in subdivisions where MatchSome($elem_qc/id, (" + ArrayMerge( arrSubIDs, "This", "," ) + ")) return $elem_qc/Fields('id')" );
		for ( iSubIDElem in arrSubIDs )
		{
			arrAllSubIDs = ArrayUnion( arrAllSubIDs, tools.xquery( "for $elem in subdivisions where IsHierChild( $elem/id, " + iSubIDElem + " ) order by $elem/Hier() return $elem/Fields('id')" ) );
		}
		...
		else
		{
			...
			var xarrPositions = tools.xquery( "for $elem_qc in positions where MatchSome( $elem_qc/parent_object_id, (" + ArrayMerge( arrAllSubIDs, "id", "," ) + ") ) return $elem_qc" );
			var iCountPositions = ArrayCount(xarrPositions);
			var xarrDismissedPositions = tools.xquery( "for $elem_qc in positions where MatchSome( $elem_qc/parent_object_id, (" + ArrayMerge( arrAllSubIDs, "This", "," ) + ") ) and some $coll in collaborators satisfies ($elem_qc/basic_collaborator_id = $coll/id and $coll/is_dismiss = true() and $coll/dismiss_date > " + XQueryLiteral(dTurnoverPeriodStart) + " and $coll/dismiss_date < " + XQueryLiteral(dTurnoverPeriodEnd) + ") return $elem_qc" );
	...
	if(arrCaclulateParam.indexOf("management_object" ) >= 0)
	{
		...
		oContext.SetProperty("has_many_management_object", (ArrayCount(get_boss_management_objects( iPersonID, true )) > 1));
	...
	if(arrCaclulateParam.indexOf("bosspanel_frame_rule" ) >= 0)
	{
		...
		{
			var arrBossRootManagementObjects = ArraySelect(get_boss_management_objects( iPersonID ), "!This.HasProperty('parent_id') || OptInt(This.GetOptProperty('parent_id', null)) == undefined");
			var arrManagementObjects = ArrayExtract(arrBossRootManagementObjects, "({id: This.id, name: This.name, rules: []})");
...
function IsBossCurUser(iCurUserIDParam, iObjectIDParam, arrBossTypesID )
{
	...
	{
		oRes.error = 501;
		oRes.errorText = "Не удалось получить ID текущего пользователя";
		return oRes;
	}
	...
	if(iObjectID != null)
	{
		...
		{
			oRes.error = 502;
			oRes.errorText = StrReplace("Объект с  ID: [{PARAM1}] не найден", "{PARAM1}", iObjectID);
			return oRes;
		}
		var teObject = docObject.TopElem;
 ...
 * @param {boolean} bAddManagementObjectClearCommand - добавить команду на очистку объекта управленичя
 * @param {number} iMaxCnt - максимальное количество записей
 * @returns {WTBossManagementObjectsResult}
 */
function GetBossManagementObjects( iPersonID, bHierSubdivisions, arrObjectTypes, bAddManagementObjectClearCommand, iMaxCnt )
{
	var arrFullManagementObjects = get_boss_management_objects( iPersonID, bHierSubdivisions, iMaxCnt );
	...
	if(bAddManagementObjectClearCommand) 
		arrAdd.push({id: 0, name: "Сброс объекта управления", type: "clear", type_name: "Сброс", temp_parent_id: "" });
	
	arrAdd.push({id: 9999, name: "Все подчиненные", type: "all", type_name: "Все", temp_parent_id: "" });
...
function get_boss_management_objects( iPersonID, bHierSubdivisions, iMaxCnt )
{
	...
	function get_hier_subdivisions( iSubdivisionID, iOrgID )
	{
		...
		if( iOrgID != null )
		{
			var xarrHierSubdivisions = ArraySelectAll(tools.xquery( "for $elem in subdivisions where $elem/org_id = " + iOrgID + " return $elem/Fields('id', 'name', 'parent_object_id', 'org_id')" ));
		}
		else
		{
			var xarrHierSubdivisions = ArraySelectAll(tools.xquery( "for $elem in subdivisions where IsHierChild( $elem/id, " + iSubdivisionID + " ) order by $elem/Hier() return $elem/Fields('id', 'name', 'parent_object_id', 'org_id')" ));
		}
	...
	if(iPersonID == undefined)
	{
		throw "Некорректный ID сотрудника";
	}
	...
	if( iMaxCnt > 0 && iCurrentCount < iMaxCnt && bHierSubdivisions )
	{
		...
		while( ArrayOptFirstElem( arrNextArray ) != undefined )
		{
			...
			{
				xarrTempArray = tools.xquery( "for $elem in subdivisions where MatchSome( $elem/org_id, ( " + ArrayMerge( arrOrgArray, "This.id", "," ) + " ) ) and $elem/parent_object_id = null() return $elem/Fields('id', 'name', 'parent_object_id', 'org_id')" );
			...
			{
				xarrTempArray = tools.xquery( "for $elem in subdivisions where MatchSome( $elem/parent_object_id, ( " + ArrayMerge( arrSubArray, "This.id", "," ) + " ) ) return $elem/Fields('id', 'name', 'parent_object_id', 'org_id')" );
...
/*	if( tools_web.is_true( libParam.GetOptProperty( "bGetOwnSubdivisionAsDefaultManagementObject", false ) ) )
	{
		...
		{
			var itemAll = ArrayFind(arrResult, "This.id == 0");
...
{ 
	...
	if(bGetFactualSubordinate && (oCurrentManagementObject == null || oCurrentManagementObject.catalog == "all"))
	{
		...
		{
			sResultReq = "for $elem_qc in person_hierarchys where MatchSome($elem_qc/" + sSubdivisionLinkField + ",(" + ArrayMerge(aCatalogFMs, "This.parent_id.Value", ",") + ")) and $elem_qc/collaborator_id != " + iPersonID + " return $elem_qc/Fields('collaborator_id')";
...
// Xquery condition отбора подчиненных для стандартного системного диалогового окна.
	...
	if(iPersonID == undefined)
		throw "ID руководителя не является целым числом";
 ...
 * @returns {XmElem[]|bigint[]}
 */
function GetTypicalSubordinates(iPersonID, sTypeSubordinate, arrBossTypesID, bReturnIDs, sXQueryCond )
{
...
function DeleteObjects( sCatalogName, SCOPE_WVARS, PARAMETERS, iUserID, teUser )
{
	...
	function get_confirm_title( iCount )
	{
		switch( sCatalogName )
		{
			...
			case "qualification":
				if (sParam == "reward"){
					_text = iCount == 1 ? "награду" : "награды";
				}
				else if (sParam == "badge"){
					_text = iCount == 1 ? "бейдж" : "бейджи";
				}
				return ( iCount == 1 ? "Вы действительно хотите удалить " + _text + "?" : "Вы действительно хотите удалить " + _text + "?" );
			case "qualification_assignment":
				if (sParam == "reward"){
					_text = iCount == 1 ? "награду" : "награды";
				}
				...
				return ( iCount == 1 ? "Вы хотите удалить " + _text + "?" : "Вы хотите удалить " + _text + "?" );
			case "policy_type":
			...
			case "attorney":
				return ( iCount == 1 ? "Вы действительно хотите удалить доверенность?" : "Вы действительно хотите удалить доверенности?" );
			case "collaborator_schedule":
				return ( iCount == 1 ? "Вы действительно хотите удалить график сотрудника?" : "Вы действительно хотите удалить графики сотрудников?" );
			case "interval_schedule":
				return ( iCount == 1 ? "Вы действительно хотите удалить интервал графика?" : "Вы действительно хотите удалить интервалы графиков?" );
			case "restricting_collaborator_schedule":
				return ( iCount == 1 ? "Вы действительно хотите удалить ограничение графика сотрудника?" : "Вы действительно хотите удалить ограничения графиков сотрудников?" );
			case "absence_reserve":
				return ( iCount == 1 ? "Вы действительно хотите удалить резерв на отсутствие?" : "Вы действительно хотите удалить резервы на отсутствие?" );
			case "schedule_type":
				return ( iCount == 1 ? "Вы действительно хотите удалить тип графика?" : "Вы действительно хотите удалить типы графиков?" );
			case "presence_state":
				return ( iCount == 1 ? "Вы действительно хотите удалить тип присутствия/отсутствия?" : "Вы действительно хотите удалить типы присутствия/отсутствия?" );
			case "check_schedule":
				return ( iCount == 1 ? "Вы действительно хотите удалить проверку графика?" : "Вы действительно хотите удалить проверки графиков?" );
			case "restricting_type":
				return ( iCount == 1 ? "Вы действительно хотите удалить тип ограничения?" : "Вы действительно хотите удалить типы ограничения?" );
		}
		return "";
	}
	function get_end_title( iCount, iDelCount )
	{
		switch( sCatalogName )
		{
			...
			case "qualification":
				if (sParam == "reward"){
					_text = "Награды";
					_finish_text = "Удалено наград: {PARAM1} из {PARAM2}. Остальные удалить невозможно, так как они уже были использованы"
				}
				else if (sParam == "badge"){
					_text = "Бейджи";
					_finish_text = "Удалено бейджей: {PARAM1} из {PARAM2}. Остальные удалить невозможно, так как они уже были назначены пользователям";
				}
				return ( iDelCount == iCount ? _text + " удалены" : StrReplace( StrReplace( _finish_text, "{PARAM1}", iDelCount ), "{PARAM2}", iCount ) );
			case "qualification_assignment":
				if (sParam == "reward"){
					return "Награды удалены";
				}
				else if (sParam == "badge"){
					return "Бейджи удалены";
				}
			case "policy_type":
	...
	catch( ex )
	{
		oRes.action_result = {
			command: "alert",
			msg: "Передан некорректный ID сотрудника",
		};
		return oRes;
	}
	try
	...
	if( ArrayOptFirstElem( arrObjectIDs ) == undefined )
	{
		if(sCatalogName == "qualification_assignment")
		{
			oRes.action_result = {
				command: "close_form",
				msg: "Сначала выберите присвоенный бейдж или награду"
			}
		}
		else
		{
...
function CreateUpdateGroup( SCOPE_WVARS, PARAMETERS )
{
	...
	if( sApplication != "" )
	{
		...
		if( curApplication != null )
		{
			...
			{
				aBossTypes = ArrayMerge(ParseJson(curApplication.wvars.GetOptChildByKey( 'manager_type_id' ).value.Value), "This.__value", ",").split(",");
...
//фильтрация
	...
	for ( catCollab in xarrCollabs )
	{
		oElem = {
			...
			consent_kedo: ( catCollab.consent_kedo ? "Да" : "Нет" ),
 ...
 * @param {string} sAccessType Тип доступа: "admin"/"manager"/"hr"/"observer"/"auto"
 * @param {bigint[]} arrBossTypesID - Массив типов руководителей
 * @param {string|bigint} ApplicationParam код или ID приложения, по которому определяется доступ
...
function GetGroupsApp( sAccessType, arrBossTypesID, sApplicationID, iCurApplicationID, sGroupType, sXQueryQual, oCollectionParams ){
	...
	/*
		Поддержать множественный выбор в параметре manager_type_id приложения. 
		То есть если в этом параметре указан массив типов функциональных руководителей, 
		то нужно учитывать подчиненных по всем этим типам. 
		Это реализовать для ролей со значениями 3 и 5.
	*/

	switch(sAccessType)
	{
		case "hr": // 5 - руководитель групп
 ...
 * @author IG
 * @description Получение списка групп.
 * @param {integer} iApplLevel - Роль, для которой отображается информация.
 * @param {bigint[]} [arrBossTypesID] - Массив типов руководителей
 * @param {string} sApplicationID код приложения, по которому определяется доступ
 * @param {bigint} iCurApplicationID ID текущего приложения
 * @param {string} sXQueryQual строка для XQuery-фильтра
...
*/
function GetSubdivisionsApp( iApplLevel, arrBossTypesID, sApplicationID, iCurApplicationID, sXQueryQual, oCollectionParams, sSubdivisionGroupID ){
	...
	switch(iApplLevel)
	{
		case 5: // HR
			...
			if (ArrayOptFirstElem(arrBossType) == undefined)
			{
				arrBossType = arrBossType = tools.call_code_library_method( "libApplication", "GetApplicationHRBossTypes", [ iApplicationID, iPersonID ] );;
			}
		...
		case 1: // наблюдатель
			...
			xqArrSubdivisionFM = ArrayDirect( tools.xquery(sSubdivisionQuery) );
	...
	{
		//иерархия
		var arrSubdivisionWithHier = [], arrSubdivHier = [];
		for ( itemSubdivision in xqArrSubdivisionFM )
		{
			if ( ArrayOptFindByKey( arrSubdivisionWithHier, itemSubdivision.Value, "id" ) != undefined )
				continue;

			arrSubdivHier = ArrayDirect( tools.xquery( "for $elem in subdivisions where IsHierChildOrSelf( $elem/id, " + itemSubdivision.Value + " ) order by $elem/Hier() return $elem/Fields('id')" ) );
...
// SEARCH
	...
	sSubdivFmQuery = "for $elem in func_managers where MatchSome( $elem/parent_id, ( " + ArrayMerge(arrSubdivisions, 'OptInt(This.id.Value, 0)', ',') + " ) ) and $elem/catalog='position' order by $elem/parent_id return $elem/Fields('parent_id','person_id')";
	var xarrSubdivFmQuery = ArrayDirect( tools.xquery(sSubdivFmQuery) );

	sSubdivBossFullnames = "for $elem in collaborators where MatchSome( $elem/id, ( " + ArrayMerge(xarrSubdivFmQuery, 'OptInt(This.person_id.Value, 0)', ',') + " ) ) order by $elem/id return $elem/Fields('id','fullname')";
...
function ExclusionReasonDelete( arrObjectIDs )
{
	...
	{
		oRes.error = 503;
		oRes.errorText = "Массив не является массивом ID оснований для исключения";
		return oRes;
	}
...
function GetRequestApps( iCurUserID, sAccessType, sApplication, iCurApplicationID, iReqTypeID, sStates, sReturnData, sXQueryQual, oCollectionParams )
{
	...
	switch(sAccessType)
	{
		...
		case "expert":
			{
				oRes.error = 501;
				oRes.errorText = 'Отсутствуют права доступа';
				return oRes;
				break;
			}
		case "observer":
...
function HasObjectInRole(iObjectIDParam)
{
	...
	try
	{
		var iObjectID = OptInt(iObjectIDParam);
		if(iObjectID == undefined)
			throw StrReplace("Не указан корректный ID объекта: [{PARAM1}]", "{PARAM1}", iObjectIDParam);
		
		var docObject = tools.open_doc(iObjectID);
		if(docObject == undefined)
			throw StrReplace("Не найден объект по ID: [{PARAM1}]", "{PARAM1}", iObjectID);
...
*/
function GetHighEffectivenessPas( arrPersonsIDs, arrAssessmentAppraiseIDs )
{
	var oRes = tools.get_code_library_result_object();
	oRes.array = [];
	...
	{
		oRes.error = 502;
		oRes.errorText = "В массиве сотрудников нет ни одного целочисленного ID";
		return oRes;
	}
...
function RequestDelete( arrRequestIDs ){
	...
	{
		oRes.error = 503;
		oRes.errorText = "Данные не являются массивом ID заявок";
		return oRes;
	}
...
function get_snils_str( sSnils )
{
	...
	if( OptInt( sSnils ) == undefined || StrLen( sSnils ) != 8 )
	{
		oRes.error = 1;
		oRes.errorText = "Некорректный снилс";
		return oRes;
	}
	sSnils = String( sSnils );
	oRes.snils_str = StrRangePos( sSnils, 0, 3 ) + "-" + StrRangePos( sSnils, 3, 6 ) + " " + StrRangePos( sSnils, 6, 8 );
...
{	
	...
	sReqAssessments = "for $elem in assessment_appraises where $elem/status = '0' and MatchSome($elem/id, (" + ArrayMerge(xarrPas, 'This.assessment_appraise_id', ',') + ")) and $elem/end_date > date(" + XQueryLiteral(Date()) + ") order by $elem/end_date descending return $elem";
	xarrAssessments = tools.xquery(sReqAssessments);
	...
	obj = {
		"start_date": oCurrentPersonAssessment.start_date,
		"end_date": oCurrentPersonAssessment.end_date,
		"assessment_appraise_type": sApprTypeName,
		"name": oCurrentPersonAssessment.name,
	}
...
function Report_SubdivWOSubdivGroup(oParams, xmCurUser)
{
	...
	tpProgress.TaskName = 'Загружаем список подразделений';
...
function Report_PositionCommonWOFamily(oParams, xmCurUser)
{
	...
	{
		oRes.error = 1;
		oRes.errorText = "Пустые параметры вызова"
		return oRes
	}
	...
	{
		oRes.error = 1;
		oRes.errorText = "Не передан текущий пользователь"
		return oRes
	}
...
//alert('for $elem in position_commons ' + sXQCond + ' return $elem')
	...
	var tpProgress = new TaskProgress;
	tpProgress.TaskName = 'Загружаем список типовых должностей';
...
function Report_PositionWOPositionCommon(oParams, xmCurUser)
{
	...
	var tpProgress = new TaskProgress;
	tpProgress.TaskName = 'Загружаем список должностей';
...
function Report_ObjectsWOTypicalProgram(oParams, xmCurUser)
{
	...
	var tpProgress = new TaskProgress;
	tpProgress.TaskName = 'Загружаем список объектов';
...
function Report_PositionCommonStatistic(oParams, xmCurUser)
{
	...
	var tpProgress = new TaskProgress;
	tpProgress.TaskName = 'Загружаем список статистики';
	...
	oRes.array.push( {'name': 'Типовых должностей', 'value': ArrayCount(arrFullPositionCommon)} );
	tpProgress.CurrentItemIndex++;
	...
	oRes.array.push( {'name': 'Типовых должностей без типовых программ', 'value': ArrayCount(arrPositionCommonWoTDP)} );
	tpProgress.CurrentItemIndex++;
	...
	oRes.array.push( {'name': 'Должностей без типовых', 'value': ArrayCount(arrPosition)} );
	tpProgress.CurrentItemIndex++;
	
	var arrFullPositionFamilys = XQuery( 'for $elem in position_familys return $elem' );
	oRes.array.push( {'name': 'Семейств должностей', 'value': ArrayCount(arrFullPositionFamilys)} );
	tpProgress.CurrentItemIndex++;
	...
	oRes.array.push( {'name': 'Семейств должностей без типовых программ', 'value': ArrayCount(arrPositionFamilyWoTDP)} );
	tpProgress.CurrentItemIndex++;

	var arrPositionCommonWoFamily = ArraySelect(arrFullPositionCommon, "This.position_familys == null || This.position_familys == ''");
	oRes.array.push( {'name': 'Типовых должностей без семейств', 'value': ArrayCount(arrPositionCommonWoFamily)} );
	tpProgress.CurrentItemIndex++;

	var arrFullSubdivisionGroup = XQuery( 'for $elem in subdivision_groups return $elem' );
	oRes.array.push( {'name': 'Групп подразделений', 'value': ArrayCount(arrFullSubdivisionGroup)} );
	tpProgress.CurrentItemIndex++;
	...
	oRes.array.push( {'name': 'Групп подразделений без типовых программ', 'value': ArrayCount(arrSubdivisionGroupWoTDP)} );
	tpProgress.CurrentItemIndex++;
	...
	oRes.array.push( {'name': 'Подразделений без групп', 'value': ArrayCount(arrSubdivisionsWoSubdivisionGroup)} );
	tpProgress.CurrentItemIndex++;
...
function Report_Person_WO_Adaptations(oParams, xmCurUser)
{
	...
	{
		oRes.error = 1;
		oRes.errorText = "Отчет вызван вне приложения"
		return oRes
	}
	var teApplication = tools_app.get_application(APPLICATION);
	if( teApplication == null )
	{
		oRes.error = 1;
		oRes.errorText = "Не найдено указанное приложение: [" + APPLICATION + "]"
		return oRes
	}
...
//alert("Report: libMain: Report_Adaptations: REQ: for $elem_qc in career_reserves" + sXQCondReq + " return $elem_qc")
	...
	var oTotal = {name: '*Всего*', code: 'total', count: 0};
