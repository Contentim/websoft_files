...
function get_socket_url( sPortalUrl, aUIDs )
{
	...
	try
	{
		...
		if( tools.spxml_unibridge.HasValue )
		{
			try
			{
				info = tools.spxml_unibridge.Object.provider.GetRunningInfo("instance_info", aUIDs);
	...
	var sUrl = undefined;
	if( t_info.GetOptProperty( "All" ) != undefined )
	{
		if( t_info.All.GetOptProperty( "Urls" ) != undefined )
		{
			if(  sPortalUrl != "" )
			{
				sUrl = ArrayOptFind( t_info.All.Urls, "StrBegins( This, " + XQueryLiteral( String( ( UrlSchema( sPortalUrl ) == "https" ? "wss://" : "ws://" ) ) ) + " )" );
			}
			else
			{
				sUrl = ArrayOptFind( t_info.All.Urls, "StrBegins( This, 'wss://' ) || StrBegins( This, 'ws://' )" );
			}
			if( sUrl != undefined )
			{
...
function GetBossSubordinateContext( iPersonID, arrCaclulateParam, dPeriodStart, dPeriodEnd, PersonnelTurnoverPeriod, bWithoutFuncManagers )
{
	...
	if(arrCaclulateParam.indexOf("percent_bosses" ) >= 0)
	{
		...
		//arrBosses = ArraySelectDistinct( arrBosses, "This.person_id" );
	...
	if(arrCaclulateParam.indexOf("percent_turnover" ) >= 0)
	{
		...
		else
		{
			...
			var xarrPositions = tools.xquery( "for $elem_qc in positions where MatchSome( $elem_qc/parent_object_id, (" + sMergeSubIDs + ") ) and MatchSome($elem_qc/basic_collaborator_id, (" + sSubordinateIDs + ")) return $elem_qc/Fields('id')" );
	...
	if(arrCaclulateParam.indexOf("dismiss_count" ) >= 0)
	{
		...
		if( sMergeSubIDs == null )
		{
			if( arrAllSubIDs == null )
			{
				arrAllSubIDs = [];
				if( arrPersonFuncManagers == null )
				{
		...
		{
			xarrDismissedCollaborators = tools.xquery( "for $elem_qc in collaborators where MatchSome( $elem_qc/position_parent_id, (" + sMergeSubIDs + ") ) and $elem_qc/is_dismiss = true() return $elem_qc/Fields('id')" );
	...
	if(arrCaclulateParam.indexOf("bosspanel_frame_rule" ) >= 0)
	{
		...
		if( oCurrentManagementObject.object_id == 9999 )
		{
			...
			oCacheData = oResBossManagementObjects.cache_data;
			var arrBossRootManagementObjects = ArraySelect( oResBossManagementObjects.result, "!This.HasProperty('parent_id') || OptInt(This.GetOptProperty('parent_id', null)) == undefined");
			...
			for(itemMO in arrBossRootManagementObjects)
			{
				...
				switch( itemMO.type )
				{
					case "org":
					{
						...
						arrTempBossTypes = ArraySelect( arrTempBossTypes, "ArrayOptFirstElem( This.object_type ) == undefined || ArrayOptFind(This.object_type, 'This.Value == &quot;org&quot;') != undefined" )
						break;
					}
					case "subdivision":
					{
			...
			for(itemOperation in arrResultOperations)
			{
				...
				arrActionRuleCodes.push(oCastOperation);
			}
		}
		else
		{
...
function get_boss_management_objects( iPersonID, bHierSubdivisions, iMaxCnt, oCacheData )
{
...
/*	if( tools_web.is_true( libParam.GetOptProperty( "bGetOwnSubdivisionAsDefaultManagementObject", false ) ) )
	{
		if(ArrayOptFirstElem(tools.xquery("for $elem in person_hierarchys where MatchSome($elem/parent_sub_id, (" + ArrayMerge(oRes.array, "This.id", ",") + ")) and $elem/collaborator_id = " + iPersonID + " return $elem")) != undefined)
		{
			...
			itemAll.name = i18n.t( 'sobstvennoepod' );
		}
	}
*/
	if( bReturnObject )
	...
	if(bGetFactualSubordinate && (oCurrentManagementObject == null || oCurrentManagementObject.catalog == "all"))
	{
		...
		if(bGetOrgSubordinate)
		{
			...
			if(ArrayOptFirstElem(aCatalogFMs) != undefined)
			{
				...
				sResultReq = "for $elem in positions where MatchSome($elem/org_id, (" + ArrayMerge( aCatalogFMs, "This.org_id.Value", "," ) + ")) and $elem/basic_collaborator_id != " + iPersonID + " and $elem/is_position_finished != true() and ($elem/position_date = null() or $elem/position_date <= " + XQueryLiteral(Date()) + ") and ($elem/position_finish_date = null() or $elem/position_finish_date >= " + XQueryLiteral(DateNewTime(Date())) + ") return $elem/Fields('basic_collaborator_id')";
